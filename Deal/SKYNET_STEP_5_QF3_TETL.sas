...
PROC SQL;
   ...
   SELECT /* PRODUCT */
      ...
      WHERE t1.PRODUCT_TYPE = 'ILP' AND t2.ST_PVC_CD NOT IS MISSING;
%RUNQUIT(&job,&sub5);
...
PROC SQL;
   CREATE TABLE QF3_TETL_DAILY_UPDATE AS 
   SELECT t1.PRODUCT, 
          t1.POS, 
          t1.INSTANCE, 
          t1.BANKMODEL, 
...
DATA _NULL_;
    ...
    CALL SYMPUTX('PATH',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DEAL\QFUND3\TETL",'G');
    CALL SYMPUTX('PATHTWO',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DEAL\QFUND3\TETL\DIR2\",'G');
%RUNQUIT(&job,&sub5);

PROC SQL;
    INSERT INTO SKY.DEALSUM_DATAMART_QF3TETL (BULKLOAD=YES BL_LOG="&PATH.\BL_&DATE..LOG" BL_DELETE_DATAFILE=YES 
...
/*UPLOAD QF3TETL*/
%INCLUDE "&DEAL_FILE_PATH.\DEALSUM_UPLOAD_QF3TETL.SAS";
