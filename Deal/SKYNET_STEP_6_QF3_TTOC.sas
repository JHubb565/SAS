...
PROC SQL;
   CREATE TABLE WORK.TTOC_LOANS AS 
   SELECT t1.LOAN_ID, 
          t1.LOAN_CODE, 
          t1.BO_CODE, 
          t1.ST_CODE, 
      ...
      WHERE t1.PRODUCT_TYPE = 'TTOC' AND t1.DATE_UPDATED >= &lastweek
      ORDER BY t1.LOAN_CODE,
               t1.LOAN_ID;
%RUNQUIT(&job,&sub6);

DATA WORK.MOST_RECENT_TTOC;
	SET WORK.TTOC_LOANS;
...
PROC SQL;
   CREATE TABLE WORK.ADD_LOCATION_DATA AS 
   SELECT /* PRODUCT */
            ("TITLE") AS PRODUCT, 
          /* POS */
            ("QFUND") AS POS, 
...
PROC SQL;
   CREATE TABLE WORK.SORT_FOR_MOST_RECENT AS 
   SELECT t1.BO_ID, 
          t1.BO_CODE, 
          t1.BO_ST_CODE, 
          t1.BUSINESS_UNIT, 
...
DATA WORK.MOST_RECENT_CUST;
	SET WORK.SORT_FOR_MOST_RECENT;
	BY BO_CODE BO_ID;
	IF LAST.BO_CODE;
%RUNQUIT(&job,&sub6);

PROC SQL;
   CREATE TABLE QF3_TTOC_DAILY_UPDATE AS 
   SELECT t1.PRODUCT, 
          t1.POS, 
          t1.INSTANCE, 
          t1.BANKMODEL, 
          ...
          t1.ACHAUTHFLG, 
          t1.ENDDT
      FROM WORK.ADD_LOCATION_DATA t1
           INNER JOIN WORK.MOST_RECENT_CUST t2 ON (t1.CUSTNBR = t2.BO_CODE);
%RUNQUIT(&job,&sub6);
...
DATA _NULL_;
    ...
    CALL SYMPUTX('PATH',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DEAL\QFUND3\TTOC",'G');
    CALL SYMPUTX('PATHTWO',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DEAL\QFUND3\TTOC\DIR2\",'G');
%RUNQUIT(&job,&sub6);

PROC SQL;
    INSERT INTO SKY.DEALSUM_DATAMART_QF3TTOC (BULKLOAD=YES BL_LOG="&PATH.\BL_&DATE..LOG" BL_DELETE_DATAFILE=YES 
...
/*UPLOAD QF3TTOC*/
%INCLUDE "&DEAL_FILE_PATH.\DEALSUM_UPLOAD_QF3TTOC.SAS";
