...
%MACRO DEAL_TRAN_DEPENDENCIES();
	...
	%DO %UNTIL (%EVAL(&COUNT_FINISHED. >= 34));
				...
				PROC SQL;
					...
					WHERE SOURCE IN ('BIOR.O_DEALTRANSACTION_ALL','BIOR.O_DEAL_SUMMARY_ALL')
				;
				QUIT;
	...
	/* KICK OFF CUSTOMER STATS DEPENDENCY DRIVER */
	SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
					 '&SKYNETREDESIGN.\CUSTOMER STATS DEPENDENCY.SAS'
					 -LOG '&SKYNETREDESIGN_LOGS.\CUST_STATS_DEPENDENCY_&TIMESTAMP..LOG'
					 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
					 TASKNAME=CSD
					 STATUS=CSD;

	/* KICK OFF CUST LIFECYCLE */
	SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
					 '&CUST_FILE_PATH.\CUST LIFE & DAILY DRIVER.SAS'
					 -LOG 'E:SHARED\CADA\LOGS\SKYNET_CUST_AND_TRANS_CUST_LIFECYCLE_DAILY_&TIMESTAMP..LOG'
					 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
					 TASKNAME=CUSTLIFE
					 STATUS=CUSTLIFE;

	WAITFOR _ALL_ CUSTLIFE CSD;

	/* KICK OFF SILVERPOP */
	SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
					 'E:\SHARED\CADA\SAS SOURCE CODE\PRODUCTION\MARKETING\SILVERPOP_DRIVER.SAS'
					 -LOG 'E:SHARED\CADA\LOGS\SILVERPOP_DAILY_SILVERPOP_DAILY_&TIMESTAMP..LOG'
					 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
					 TASKNAME=SPOP
					 STATUS=SPOP;

	/* KICK OFF OMNIDATA DASHBOARD */
	SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
					 'E:\SHARED\CADA\SAS SOURCE CODE\PRODUCTION\OMNI DATA\OMNI_CHANNEL_DASHBOARD.SAS'
					 -LOG 'E:SHARED\CADA\LOGS\OMNI_CHANNEL_DASHBOARD_&TIMESTAMP..LOG'
					 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
					 TASKNAME=OMNI
					 STATUS=OMNI;
	...
	%IF %EVAL(&MONDAY_OR_NOT. ^= 2) %THEN
		%DO;
			PROC SQL;
				...
				WHERE TABLE_DEPENDENT_COUNT ^= 1 AND (DEPENDENCIES = 'BIOR.O_DEAL_SUMMARY_ALL' OR DEPENDENCIES = 'BIOR.O_DEALTRANSACTION_ALL')
			;
			QUIT;
			...
			FILENAME EML EMAIL TO=('BI_DATA@ADVANCEAMERICA.NET','BI_REPORTING@ADVANCEAMERICA.NET') CC=('SHOPKINS@ADVANCEAMERICA.NET') SUBJECT="DEAL/TRAN DEPENDENT TDE'S KICKED OFF &TDY" CT='TEXT/HTML';
			ODS LISTING CLOSE;
			ODS HTML BODY=EML;
			...
			PROC SQL;
			TITLE "DEAL/TRAN DASHBOARD TDE'S KICKED OFF";
				SELECT A.*
