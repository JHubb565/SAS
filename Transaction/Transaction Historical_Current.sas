...
/*SEE IF ITS A SUNDAY AND IF IT'S BEEN A MONTH. IF ITS A SUNDAY AND IT HAS BEEN A MONTH WE WILL MOVE DATA FROM TRAN TO HISTORIC TRAN*/
	...
	%IF %EVAL(&RUN. = 1 AND &DAY. = 1) %THEN 
		%DO;
			...
			PROC SQL;
				...
				CREATE TABLE ODT_CURRENT AS
				SELECT *
				FROM CONNECTION TO ORACLE
				(
					SELECT *
					FROM BIOR.O_DEALTRANSACTION_90
					WHERE BUSINESSDT < TRUNC(CURRENT_DATE)-90
				)
				;
			...
			PROC SQL;
			    INSERT INTO BIOR.O_DEALTRANSACTION_HIS (BULKLOAD=YES BL_LOG="&PATH.\BL_&DATE..LOG" BL_DELETE_DATAFILE=YES 
			...
			PROC SQL;
				CONNECT TO ORACLE (USER=&USER. PASSWORD=&PASSWORD. PATH='BIOR');
				EXECUTE (DELETE FROM BIOR.O_DEALTRANSACTION_90
						 WHERE BUSINESSDT < TRUNC(CURRENT_DATE)-90) BY ORACLE;
				DISCONNECT FROM ORACLE;
			QUIT;
			...
			DATA _NULL_;
				...
				SUBJECT="TRANSACTION HISTORICAL TRANSFER COMPLETE";
			RUN;
