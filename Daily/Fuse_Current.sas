...
PROC SQL;
	CREATE TABLE WORK.DM_DAILY_SUMMARY_TMPX1 AS 
		...
		WHERE T1.SOURCE_SYSTEM_CD = 'QFUND_X' AND T1.BUSINESS_DATE >= DHMS(TODAY()-5,0,0,0)
				GROUP BY
						 T1.BUSINESS_DATE,
						 T1.LOC_NBR,
						 T1.DESCRIPTION,
...
CREATE TABLE WORK.CUSTOMER_NUMBERS AS 
   SELECT 
        DHMS(DATEPART(t1.BUSINESS_DATE),0,0,0) FORMAT=DATETIME. AS BUSINESSDT, 
        t1.LOCATION_NBR, 
        t1.INSTANCE, 
        t1.PRODUCT, 
	...
	WHERE T1.INSTANCE = 'FUSE' AND T1.BUSINESS_DATE >= DHMS(TODAY()-5,0,0,0)
...
CREATE TABLE WORK.FUSE_DAILY_FINAL AS
	...
	FROM WORK.DAILYSUM_DATAMART_FUSE_TMP T1
		LEFT JOIN WORK.CUSTOMER_NUMBERS t2 ON (T1.LOCNBR = T2.LOCATION_NBR AND T1.PRODUCT = T2.PRODUCT AND T1.BUSINESSDT = T2.BUSINESSDT);
QUIT;
...
SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
				 '&DAILY_FILE_PATH.\TRANSPOSE FUSE.SAS'
