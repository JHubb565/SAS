...
PROC SQL;
   CREATE TABLE WORK.PD_FOR_ETL AS 
   SELECT t1.ST_CODE AS LOCNBR, 
      ...
      FROM TETL.PAST_DUE_ILP t1
      WHERE t1.PRODUCT_TYPE = 'FAI'
      GROUP BY t1.ST_CODE,
...
PROC SQL;
   CREATE TABLE WORK.tetl_initial_dailysummary AS 
   SELECT /* Product */
            ('INSTALLMENT') LABEL="Product" AS Product, 
          /* pos */
          ...
          /* advcnt */
            (SUM(t1.NEW_ORIGINATION_CNT)) AS advcnt, 
          /* advamtsum */
            (SUM(t1.NEW_ORIGINATION_AMT)) FORMAT=12.2 AS advamtsum, 
          /* newcustdealcnt */
            (SUM(t1.NEW_CUSTOMER_CNT)) AS newcustdealcnt, 
          /* totadvrecv */
            (SUM(t1.LOAN_RECEIVABLE_AMT)) FORMAT=12.2 AS totadvrecv, 
          /* totadvfeerecv */
            (SUM(t1.CSO_FEE_RECEIVABLE_AMT)) FORMAT=12.2 AS totadvfeerecv, 
          /* defaultcnt */
            (SUM(t1.DEFAULT_CNT)) AS defaultcnt, 
          /* DEFAULTAMT */
            (SUM(t1.DEFAULT_AMT)) FORMAT=12.2 AS DEFAULTAMT, 
          /* totdefaultrecv */
            (SUM(t1.DEFAULT_AMT)) FORMAT=12.2 AS totdefaultrecv, 
          /* earnedfees */
            (SUM(t1.EARNED_FEE_AMT)) FORMAT=12.2 AS earnedfees, 
          /* heldcnt */
            (SUM(t1.CURRENT_LOAN_CNT)) AS heldcnt, 
          /* woamtsum */
            (SUM(t1.WRITEOFF_AMT)) FORMAT=12.2 AS woamtsum, 
          /* WOCNT */
            (SUM(t1.WRITEOFF_CNT)) AS WOCNT, 
          /* wobamtsum */
            (SUM(t1.WRITEOFF_BANKRUPT_AMT + t1.WRITEOFF_DECEASED_AMT)) LABEL="wobamtsum" AS wobamtsum, 
          /* WOBCNT */
            (SUM(t1.WRITEOFF_BANKRUPT_CNT)) AS WOBCNT, 
          /* WODCNT */
            (SUM(t1.WRITEOFF_DECEASED_CNT)) AS WODCNT, 
          /* woramtsum */
            (SUM(t1.WRITEOFF_RECOVERY_AMT)) FORMAT=12.2 AS woramtsum, 
          /* enddt */
...
PROC SQL;
   CREATE TABLE WORK.FAI_DEFAULT_RECV AS 
   SELECT t1.TRAN_DATE, 
          t1.STORE_NUMBER, 
          t1.STATE_CODE, 
          t1.DESCRIPTION, 
          t1.TODAY_COUNT AS DEFAULT_LOANS_OUTSTANDING, 
          t1.TODAY_AMOUNT AS TOTDEFAULTRECV, 
          t1.PRODUCT_TYPE, 
          /* STD_DESC */
            (COMPRESS(UPCASE(t1.DESCRIPTION))) AS STD_DESC, 
          /* BUSINESSDT */
            (DATEPART(t1.TRAN_DATE)) FORMAT=MMDDYY10. AS BUSINESSDT
      FROM QFUND3.DAILY_CENTER_SUMMARY t1
      WHERE t1.PRODUCT_TYPE = 'FAI' AND (CALCULATED STD_DESC) = 'DEFAULTLOANS'
      ORDER BY t1.STORE_NUMBER,
               t1.TRAN_DATE;
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.tetl_schedpmtdts AS 
   SELECT t1.LOAN_ID, 
          t1.LOAN_CODE, 
          t1.INST_DUE_DATE, 
          t1.ACTIVE_FLG
      FROM TETL.LOAN_SCHEDULE t1
      WHERE t1.ACTIVE_FLG = 'Y' AND t1.PRODUCT_TYPE = 'FAI'
...
/* -------------------------------------------------------------------
   ...
   ------------------------------------------------------------------- */
PROC SORT DATA=WORK.TETL_SCHEDPMTDTS(FIRSTOBS=1 )
	OUT=WORK.tetl_lastschedpmtdt(LABEL="Sorted WORK.TETL_SCHEDPMTDTS")
	NODUPKEY
	;
	BY LOAN_CODE;
...
PROC SQL;
   CREATE TABLE WORK.tetl_loansummary_tmp1 AS 
   SELECT t1.LOAN_ID, 
          t1.LOAN_CODE, 
          t1.BO_CODE, 
          t1.ST_CODE, 
      ...
      WHERE t1.EFFECTIVE_END_DT = '31Dec9999:0:0:0'dt AND t1.LOAN_STATUS_ID NOT = 'V' AND t1.PRODUCT_TYPE = 'FAI'
      ORDER BY t1.LOAN_CODE,
               t1.LOAN_ID;
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.TETL_LOANSUMMARY_TMP2 AS 
   SELECT t1.LOAN_ID, 
          t1.LOAN_CODE, 
          t1.BO_CODE, 
          t1.ST_CODE, 
          ...
          t1.UPDATE_PROGRAM_NM, 
          t1.ACTIVE_FLG
      FROM WORK.TETL_LOANSUMMARY_TMP1 t1, WORK.TETL_LASTSCHEDPMTDT t2
      WHERE (t1.LOAN_ID = t2.LOAN_ID AND t1.LOAN_CODE = t2.LOAN_CODE);
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.tetl_durations AS 
   SELECT t1.LOAN_ID, 
          t1.LOAN_CODE, 
          t1.ST_CODE, 
          t1.LOAN_AMT, 
      ...
      FROM WORK.TETL_LOANSUMMARY_TMP2 t1;
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.TETL_DURATION_TMP1 AS 
   SELECT t1.LOAN_ID, 
          t1.LOAN_CODE, 
          t1.ST_CODE, 
          t1.LOAN_AMT, 
      ...
      FROM WORK.TETL_DURATIONS t1;
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.TETL_ACTUAL_DURATION AS 
   SELECT t1.ST_CODE AS locnbr, 
          /* repaid_duration_count */
      ...
      FROM WORK.TETL_DURATION_TMP1 t1
      GROUP BY t1.ST_CODE,
               (CALCULATED businessdt);
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.tetl_scheduled_duration AS 
   SELECT t1.ST_CODE AS locnbr, 
...
PROC SQL;
   ...
   SELECT t1.ST_CODE AS locnbr, 
          /* businessdt */
            (t1.loan_date) FORMAT=mmddyy10. AS businessdt, 
          /* ADVFEEAMT */
            (SUM(t1.CSO_FEE)) AS ADVFEEAMT
      FROM WORK.TETL_LOANSUMMARY_TMP1 t1
      GROUP BY t1.ST_CODE,
               (t1.loan_date);
%RUNQUIT(&job,&sub8);
...
PROC SQL;
   CREATE TABLE WORK.TETL_TNXS AS 
...
PROC SQL;
   CREATE TABLE WORK.QF3_TETL_PNL AS 
   SELECT t1.STORE_NUMBER AS LOCBNR, 
          /* BUSINESSDT */
      ...
      WHERE t1.SOURCE_SYSTEM = 'QFUND3' AND t1.PRODUCT_TYPE = 'TETL2';
%RUNQUIT(&job,&sub8);

PROC SQL;
   CREATE TABLE WORK.TETL_DAILYSUMMARY AS 
   SELECT t1.Product, 
          t1.pos, 
          t1.INSTANCE, 
          t1.bankmodel, 
          ...
          t1.defaultcnt, 
          t1.DEFAULTAMT, 
          t9.TOTDEFAULTRECV AS TOTDEFAULTRECV, 
          t9.DEFAULT_LOANS_OUTSTANDING AS DEFAULT_LOANS_OUTSTANDING, 
          t5.PASTDUECNT_1, 
          t5.PASTDUEAMT_1, 
          t5.PASTDUECNT_2, 
          t5.PASTDUEAMT_2, 
      ...
      FROM WORK.TETL_INITIAL_DAILYSUMMARY t1
           LEFT JOIN WORK.TETL_ACTUAL_DURATION t2 ON (t1.LOCNBR = t2.locnbr) AND (t1.businessdt = t2.businessdt)
           LEFT JOIN WORK.TETL_SCHEDULED_DURATION t3 ON (t1.LOCNBR = t3.locnbr) AND (t1.businessdt = t3.businessdt)
           LEFT JOIN WORK.ADVFEEAMT t4 ON (t1.LOCNBR = t4.locnbr) AND (t1.businessdt = t4.businessdt)
           LEFT JOIN WORK.PD_FOR_ETL t5 ON (t1.LOCNBR = t5.LOCNBR) AND (t1.businessdt = t5.BUSINESSDT) AND (t1.Product 
          = t5.PRODUCT)
           ...
           LEFT JOIN WORK.QF3_TETL_PNL t10 ON (t1.businessdt = t10.BUSINESSDT) AND (t1.LOCNBR = t10.LOCBNR)
           LEFT JOIN WORK.FAI_DEFAULT_RECV t9 ON (t1.LOCNBR = t9.STORE_NUMBER) AND (t1.businessdt = t9.BUSINESSDT);
%RUNQUIT(&job,&sub8);

PROC SQL;
	CREATE TABLE FAI_DAILYSUMMARY AS
		SELECT T1.*
			  ,CASE WHEN T2.BEGIN_PWO_AMT = . THEN 0 ELSE T2.BEGIN_PWO_AMT END AS BEGIN_PWO_AMT
		FROM WORK.TETL_DAILYSUMMARY T1
		LEFT JOIN 
...
PROC SQL;
   CREATE TABLE WORK.FAI_NEW_ORIGINATIONS AS 
   SELECT t1.Product, 
          t1.pos, 
          t1.INSTANCE, 
          t1.bankmodel, 
...
CREATE TABLE WORK.RU1_LENDINGPRODUCTS_ROLLUP_PRE AS 
	SELECT * FROM FAI_NEW_ORIGINATIONS
		OUTER UNION CORR 
	SELECT * FROM QFUND3_FAI
;
%RUNQUIT(&job,&sub8);
...
CONNECT TO ORACLE (USER=&USER. PW=&PASSWORD. PATH='BIOR');
	EXECUTE(UPDATE BIOR.DATAMART_STATUS
			SET QFUND3_FAI_STATUS = 'WAITING_CL'
...
PROC SQL;
   CREATE TABLE DAILY_SUMMARY_ALL_FAI_1 AS 
   SELECT t1.Product, 
          t1.PRODUCT_DESC, 
          t1.pos, 
          t1.INSTANCE, 
...
DATA _NULL_;
    ...
    CALL SYMPUTX('PATH',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DAILY\QFUND3\FAI",'G');
    CALL SYMPUTX('PATHTWO',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DAILY\QFUND3\FAI\Dir2",'G');
%RUNQUIT(&job,&sub8);

PROC SQL;
    INSERT INTO SKY.DAILYSUM_DATAMART_FAI (BULKLOAD=YES BL_LOG="&PATH.\BL_&DATE..LOG" BL_DELETE_DATAFILE=YES 
                                                   BL_DEFAULT_DIR="&PATHTWO.")
    SELECT 
        *
    FROM DAILY_SUMMARY_ALL_FAI_1
...
SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
				 '&DAILY_FILE_PATH.\TRANSPOSE QFUND3_FAI.SAS'
				 -LOG '&DAILY_LOGPATH.\TRANSPOSE_QFUND3_FAI_&TIMESTAMP..LOG'
				 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
TASKNAME=TRANSPOSE_QF3_FAI
STATUS=TRANSPOSE_QF3_FAI;

/*UPLOAD QF3 FAI*/
%INCLUDE "&DAILY_FILE_PATH.\DAILYSUM_UPLOAD_QFUND3_FAI.SAS";
...
PROC SQL;
	...
	EXECUTE(UPDATE BIOR.O_DAILY_SUMMARY_ALL
			...
			WHERE INSTANCE = 'QFUND3' AND PRODUCT_DESC = 'TX FAI'
			)
	BY ORACLE;
	DISCONNECT FROM ORACLE;
QUIT;

WAITFOR _ALL_ TRANSPOSE_QF3_FAI;
