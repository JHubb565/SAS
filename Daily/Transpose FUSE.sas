...
PROC SQL;
	CONNECT TO ORACLE(USER=&USER. PASSWORD=&PASSWORD. PATH='BIOR');
		EXEC(
			INSERT INTO SKYNET.TRANSPOSE_DAILY_SUMMARY_FUSE
				SELECT
				...
				FROM (
					...
					FROM (
							...
							,COMPLOANRECVAMT AS TOTADVRECV
							,COMPLOANPOTREVENUEAMT AS TOTADVFEERECV
							,COMPCNT AS COMPLIANT_LOANS_OUTSTANDING 
							,NEWCUSTCNTCOMPANY
							,NETREVENUEAMT AS NET_REVENUE
							,NEWLOANSCNT AS NEW_ORIGINATIONS 
							,GROSSREVENUEAMT AS GROSS_REVENUE
							,WOCOEXPAMT AS NET_WRITE_OFF
							,WOCOPAYAMT AS WORAMTSUM
							,DEFLOANRECVAMT AS TOTDEFAULTRECV
							,REACTIVE_CUSTOMER_CNT
			            FROM SKYNET.DAILYSUM_DATAMART_FUSE
					) 
            GROUP BY
                PRODUCT ,PRODUCT_DESC ,POS ,INSTANCE ,BRANDCD ,BANKMODEL 
                ,COUNTRYCD ,STATE ,CITY ,ZIP ,ZONENBR ,ZONENAME ,REGIONNBR 
                ,REGIONRDO ,DIVISIONNBR ,DIVISIONDDO ,LOCNBR ,LOCATION_NAME 
                ,LOC_OPEN_DT ,LOC_CLOSE_DT ,BUSINESSDT ,LAST_REPORT_DT ,LOC_LAST_REPORTED_DT
                ,LATITUDE ,LONGITUDE ,HOLIDAYNAME ,LASTTHURSDAY 
                ,THURSDAYWEEK
				,CHANNELCD
...
PROC SQL;
	CONNECT TO ORACLE (USER=&USER. PASSWORD=&PASSWORD. PATH="BIOR");
	EXECUTE (MERGE INTO BIOR.TRANSPOSE_DAILY_SUMMARY_FUSE BIOR
    USING SKYNET.TRANSPOSE_DAILY_SUMMARY_FUSE UPSERT
