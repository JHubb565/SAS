...
/*            (SUM(t1.DEFAULT_AMT)) FORMAT=12.2 AS totdefaultrecv, */
            (SUM(t1.OUTSTANDING_DEFAULT_AMT)) FORMAT=12.2 AS totdefaultrecv, 
          /* earnedfees */
...
PROC SQL;
   ...
   SELECT t1.LOAN_ID, 
      ...
      WHERE t1.EFFECTIVE_END_DT = '31Dec9999:0:0:0'dt AND t1.LOAN_STATUS_ID NOT = 'V' AND t1.PRODUCT_TYPE = 'ILP'
      ORDER BY t1.LOAN_CODE,
               t1.LOAN_ID;
%RUNQUIT(&job,&sub6);
...
PROC SQL;
   ...
   SELECT t1.STORE_NUMBER AS LOCNBR, 
          ...
          /* REFINANCE_CNT */
            (SUM(t1.CASH_AGAINS_CNT)) AS REFINANCE_CNT
      FROM QFUND3.DM_DAILY_CENTER_SUMMARY t1
      WHERE t1.TRAN_DATE >= '6Mar2017:0:0:0'dt
...
PROC SQL;
	CREATE TABLE TETL_DAILYSUMMARY AS
		SELECT T1.*
...
PROC SQL;
   CREATE TABLE TETL_NEW_ORIGINATIONS AS 
   SELECT t1.Product, 
          t1.pos, 
          t1.INSTANCE, 
          t1.bankmodel, 
...
CREATE TABLE WORK.RU1_LENDINGPRODUCTS_ROLLUP_PRE AS 
	SELECT * FROM TETL_NEW_ORIGINATIONS
		OUTER UNION CORR 
	SELECT * FROM QFUND3_TETL
;
%RUNQUIT(&job,&sub6);
...
CONNECT TO ORACLE (USER=&USER. PW=&PASSWORD. PATH='BIOR');
	EXECUTE(UPDATE BIOR.DATAMART_STATUS
			SET QFUND3_TETL_STATUS = 'WAITING_CL'
...
CONNECT TO ORACLE (USER=&USER. PW=&PASSWORD. PATH='BIOR');
	EXECUTE(UPDATE BIOR.DATAMART_STATUS
			SET QFUND3_TETL_STATUS = 'RUNNING'
			WHERE SOURCE = 'BIOR.O_DAILY_SUMMARY_ALL'
			)
	 BY ORACLE;
	 DISCONNECT FROM ORACLE;
...
PROC SQL;
   CREATE TABLE DAILY_SUMMARY_ALL_TETL_1 AS 
   SELECT t1.Product, 
          t1.PRODUCT_DESC, 
          t1.pos, 
          t1.INSTANCE, 
...
DATA _NULL_;
    ...
    CALL SYMPUTX('PATH',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DAILY\QFUND3\TETL",'G');
    CALL SYMPUTX('PATHTWO',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DAILY\QFUND3\TETL\Dir2",'G');
%RUNQUIT(&job,&sub6);

PROC SQL;
    INSERT INTO SKY.DAILYSUM_DATAMART_TETL (BULKLOAD=YES BL_LOG="&PATH.\BL_&DATE..LOG" BL_DELETE_DATAFILE=YES 
                                                   BL_DEFAULT_DIR="&PATHTWO.")
    SELECT 
        *
    FROM DAILY_SUMMARY_ALL_TETL_1
...
SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
				 '&DAILY_FILE_PATH.\TRANSPOSE QFUND3_TETL.SAS'
				 -LOG '&DAILY_LOGPATH.\TRANSPOSE_QFUND3_TETL_&TIMESTAMP..LOG'
				 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
TASKNAME=TRANSPOSE_QF3_TETL
STATUS=TRANSPOSE_QF3_TETL;

/*UPLOAD QF3 TETL*/
%INCLUDE "&DAILY_FILE_PATH.\DAILYSUM_UPLOAD_QFUND3_TETL.SAS";
...
PROC SQL;
	...
	EXECUTE(UPDATE BIOR.O_DAILY_SUMMARY_ALL
			...
			WHERE INSTANCE = 'QFUND3' AND PRODUCT_DESC = 'TX TETL'
			)
	BY ORACLE;
	DISCONNECT FROM ORACLE;
QUIT;

WAITFOR _ALL_ TRANSPOSE_QF3_TETL;
