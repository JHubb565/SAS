...
PROC SQL;
   CREATE TABLE WORK.QF5_TLP_PNL AS 
   SELECT t1.STORE_NUMBER AS LOCNBR, 
      ...
      WHERE t1.SOURCE_SYSTEM = 'QFUND5' AND (CALCULATED PRODUCT_TYPE) = 'TLP';
%RUNQUIT(&job,&sub11);

PROC SQL;
   CREATE TABLE WORK.prep_past_due_CSOTLP AS 
   SELECT t1.ST_CODE AS LOCNBR, 
          /* BUSINESSDT */
            (datepart(t1.AS_OF_DATE)) FORMAT=MMDDYY10. LABEL="BUSINESSDT" AS BUSINESSDT, 
          /* PASTDUECNT_1 */
            (SUM(t1.PASTDUE_LOAN_CNT_01_10)) FORMAT=11. AS PASTDUECNT_1, 
          /* PASTDUEAMT_1 */
            (SUM(t1.PASTDUE_LOAN_AMT_01_10)) FORMAT=12.2 AS PASTDUEAMT_1, 
          /* PASTDUECNT_2 */
            (SUM(t1.PASTDUE_LOAN_CNT_11_40)) FORMAT=11. AS PASTDUECNT_2, 
          /* PASTDUEAMT_2 */
            (SUM(t1.PASTDUE_LOAN_AMT_11_40)) FORMAT=12.2 AS PASTDUEAMT_2, 
          /* PASTDUECNT_3 */
            (SUM(t1.PASTDUE_LOAN_CNT_41_89)) FORMAT=11. AS PASTDUECNT_3, 
          /* PASTDUEAMT_3 */
            (SUM(t1.PASTDUE_LOAN_AMT_41_89)) FORMAT=12.2 AS PASTDUEAMT_3, 
          /* PASTDUECNT_4 */
            (SUM(t1.PASTDUE_LOAN_CNT_GRT90)) FORMAT=11. AS PASTDUECNT_4, 
          /* PASTDUEAMT_4 */
            (SUM(t1.PASTDUE_LOAN_AMT_GRT90)) FORMAT=12.2 AS PASTDUEAMT_4
      FROM EDW.CSO_PAST_DUE_TLP t1
      GROUP BY t1.ST_CODE,
...
PROC SQL;
	...
	CREATE TABLE WORK.WOAMT0 AS
	SELECT *
	FROM CONNECTION TO ORACLE
	(
		SELECT *
		FROM EDW.CSO_DAILY_CENTER_SUMMARY
		WHERE TRAN_DATE >= TRUNC(CURRENT_DATE)-10 AND PRODUCT_TYPE = 'TLP'
			AND DESCRIPTION IN
           (
           'Actual Write Off',
...
PROC SQL;
   ...
   SELECT t1.ST_CODE, 
      ...
      WHERE t1.DESCRIPTION IN 
           ...
           ),00,00,00) AND t1.PRODUCT_TYPE = 'TLP'
      GROUP BY t1.ST_CODE,
               t1.TRAN_DATE,
               (CALCULATED BUSINESSDT)
      ORDER BY t1.ST_CODE,
...
PROC SQL;
   ...
   SELECT t1.LOAN_ID, 
      ...
      WHERE t1.TRAN_DATE >= INTNX('MONTH',TODAY(),-24,'BEGINNING') AND t1.PRODUCT_TYPE = 'TLP';
%RUNQUIT(&job,&sub11);
...
PROC SQL;
   CREATE TABLE WORK.QFUND56_WRITEOFFS AS 
   SELECT t1.ST_CODE AS LOCNBR, 
          t1.LOAN_CODE, 
          t1.TRAN_DATE, 
          /* WOAMTSUM */
...
PROC SQL;
   ...
   SELECT t1.LOCNBR, 
      ...
      FROM WORK.QFUND56_WRITEOFFS t1
      GROUP BY t1.LOCNBR,
               t1.TRANDT
      ORDER BY t1.LOCNBR,
               t1.TRANDT;
...
PROC SQL;
   CREATE TABLE WORK.QFUND56_TITLE_DAILYSUMMARY_TMP AS 
   SELECT /* Product */
...
/*		  t1.OPEN_LOANS_AMT_SUM AS totadvrecv, */
          ...
          /* EARNEDFEES */
            (CASE WHEN t1.BUSINESS_DT < '01JUN2015:00:00:00'DT then t5.SUM_OF_CSO_FEE else t4.SUM_of_TODAY_AMOUNT end) 
            AS EARNEDFEES, 
          ...
          t2.SOLD_AMT, 
          t2.BOOK_VALUE_AMT, 
          t2.WRITEOFF_AMT AS woamtsum, 
          t6.WO_CSO_FEE, 
          /* WOAMTSUM_PRE */
            (sum(t2.WRITEOFF_AMT, -t6.WO_CSO_FEE)) AS WOAMTSUM_PRE, 
          t2.WRITEOFF_CNT AS WOCNT, 
          /* wobamtsum */
            (sum(t2.WRITEOFF_BANKRUPT_AMT,t2.WRITEOFF_DECEASED_AMT)) AS wobamtsum, 
          t6.WOB_CSO_FEE, 
          /* WOBAMTSUM_PRE */
            (SUM((sum(t2.WRITEOFF_BANKRUPT_AMT,t2.WRITEOFF_DECEASED_AMT)), -t6.WOB_CSO_FEE)) AS WOBAMTSUM_PRE, 
      ...
      FROM QFUND5.NCP_DAILYSUMMARY t1
           ...
           INNER JOIN OHCSO.DAILY_FINANCE_SUMMARY t2 ON (t1.BUSINESS_DT = t2.BUSINESS_DT) AND (t1.LOCATION_NBR = 
      ...
      WHERE (CALCULATED businessdt) BETWEEN (CALCULATED begindt) AND (CALCULATED Enddt) AND t1.PRODUCT_TYPE = 'TLP' AND 
           t3.ST_PVC_CD NOT IS MISSING;
%RUNQUIT(&job,&sub11);
...
PROC SQL;
   ...
   SELECT t1.ST_CODE AS LOCNBR, 
          ...
          /* DECAULT_CNT */
            (COUNT(t1.LOAN_CODE)) AS DECAULT_CNT
      FROM EDW.CSO_LOAN_TRANSACTION t1
      WHERE t1.PRODUCT_TYPE = 'TLP' AND t1.TRAN_ID = 'DEF'
      GROUP BY t1.ST_CODE,
               (CALCULATED BUSINESSDT);
%RUNQUIT(&job,&sub11);
...
PROC SQL;
   ...
   SELECT t1.ST_CODE AS LOCNBR, 
      ...
      FROM EDW.CSO_LOAN_TRANSACTION t1
      WHERE t1.TRAN_ID = 'DFP' AND t1.PRODUCT_TYPE = 'TLP'
      GROUP BY t1.ST_CODE,
               (CALCULATED BUSINESSDT);
%RUNQUIT(&job,&sub11);
...
PROC SQL;
   CREATE TABLE WORK.QFUND56_TITLE_DAILYSUMMARY AS 
   SELECT t1.Product, 
          t1.pos, 
          t1.INSTANCE, 
          t1.bankmodel, 
          ...
          /* PASTDUEAMT_4 */
            ...
            END) FORMAT=12.2 LABEL="PASTDUEAMT_4" AS PASTDUEAMT_4, 
          /* WOAMTSUM */
            (CASE WHEN T1.BUSINESSDT < '01JUN2015'D THEN t1.WOAMTSUM_PRE ELSE t6.WOAMTSUM END) AS WOAMTSUM, 
          /* WOCNT */
            (CASE WHEN t1.businessdt < '01JUN2015'D THEN t1.WOCNT ELSE t6.WOCNT END) AS WOCNT, 
          /* WOBAMTSUM */
            (CASE WHEN t1.businessdt < '01JUN2015'D THEN t1.WOBAMTSUM_PRE ELSE t6.WOBAMTSUM END) AS WOBAMTSUM, 
          /* WOBCNT */
            (CASE WHEN t1.businessdt < '01JUN2015'D THEN t1.WOBCNT ELSE t6.WOBCNT END) AS WOBCNT, 
          /* WORAMTSUM */
            (CASE WHEN t1.businessdt < '01JUN2015'D THEN t1.woramtsum ELSE t6.WORAMTSUM END) AS WORAMTSUM, 
          t5.CURRENT_PWO_AMT, 
      ...
      FROM WORK.QFUND56_TITLE_DAILYSUMMARY_TMP t1
           LEFT JOIN WORK.PREP_PAST_DUE_CSOTLP t2 ON (t1.LOCNBR = t2.LOCNBR) AND (t1.businessdt = t2.BUSINESSDT)
           LEFT JOIN WORK.DEFAULT_CNT_AMT t3 ON (t1.LOCNBR = t3.LOCNBR) AND (t1.businessdt = t3.BUSINESSDT)
           ...
           LEFT JOIN WORK.WOAMT t6 ON (t1.businessdt = t6.BUSINESSDT) AND (t1.LOCNBR = t6.ST_CODE)
           LEFT JOIN WORK.QF5_TLP_PNL t7 ON (t1.LOCNBR = t7.LOCNBR) AND (t1.businessdt = t7.BUSINESSDT);
%RUNQUIT(&job,&sub11);

PROC SQL;
	CREATE TABLE QFUND56_TITLE_DAILYSUMMARY AS
		SELECT T1.*
			  ,CASE WHEN T2.BEGIN_PWO_AMT = . THEN 0 ELSE T2.BEGIN_PWO_AMT END AS BEGIN_PWO_AMT
		FROM WORK.QFUND56_TITLE_DAILYSUMMARY T1
		LEFT JOIN
...
PROC SQL;
   CREATE TABLE WORK.QFUND56_TITLE_NEW_ORIGINATIONS AS 
   SELECT t1.Product, 
          t1.pos, 
          t1.INSTANCE, 
          t1.BRANDCD, 
          ...
          t1.heldcnt, 
          /* NET_WRITE_OFF_NEW */
            (SUM(t1.GROSS_WRITE_OFF,t1.WOR)) AS NET_WRITE_OFF_NEW, 
          /* GROSS_REVENUE_NEW */
            (-t1.GROSS_REVENUE) AS GROSS_REVENUE_NEW, 
          /* GROSS_WRITE_OFF_NEW */
            (t1.GROSS_WRITE_OFF) AS GROSS_WRITE_OFF_NEW, 
          /* NET_REVENUE_NEW */
            (SUM(-t1.GROSS_REVENUE,-(SUM(t1.GROSS_WRITE_OFF,t1.WOR)))) AS NET_REVENUE_NEW
      FROM QFUND56_TITLE_DAILYSUMMARY t1;
%RUNQUIT(&job,&sub11);
...
CREATE TABLE WORK.RU1_LENDINGPRODUCTS_ROLLUP_PRE AS 
	SELECT * FROM QFUND56_TITLE_NEW_ORIGINATIONS
		OUTER UNION CORR 
	SELECT * FROM QFUND5_TLP
;
%RUNQUIT(&job,&sub11);
...
PROC SQL;
   ...
   SELECT t1.Product, 
          ...
          /* NET_WRITE_OFF_NEW */
            (SUM(t1.NET_WRITE_OFF_NEW)) AS NET_WRITE_OFF_NEW, 
          /* GROSS_REVENUE_NEW */
            (SUM(t1.GROSS_REVENUE_NEW)) AS GROSS_REVENUE_NEW, 
          /* GROSS_WRITE_OFF_NEW */
            (SUM(t1.GROSS_WRITE_OFF_NEW)) FORMAT=12.2 AS GROSS_WRITE_OFF_NEW, 
          /* NET_REVENUE_NEW */
            (SUM(t1.NET_REVENUE_NEW)) AS NET_REVENUE_NEW, 
          /* WORAMTSUM_OLD */
...
CONNECT TO ORACLE (USER=&USER. PW=&PASSWORD. PATH='BIOR');
	EXECUTE(UPDATE BIOR.DATAMART_STATUS
			SET QFUND5_TITLE_STATUS = 'WAITING_CL'
...
CONNECT TO ORACLE (USER=&USER. PW=&PASSWORD. PATH='BIOR');
	EXECUTE(UPDATE BIOR.DATAMART_STATUS
			SET QFUND5_TITLE_STATUS = 'RUNNING'
			WHERE SOURCE = 'BIOR.O_DAILY_SUMMARY_ALL'
			)
	 BY ORACLE;
	 DISCONNECT FROM ORACLE;
...
PROC SQL;
   CREATE TABLE DAILY_SUMMARY_ALL_QF5T_1 AS 
   SELECT t1.Product, 
          t1.PRODUCT_DESC, 
          t1.pos, 
          "QFUND5-6"				AS INSTANCE, 
...
DATA _NULL_;
    ...
    CALL SYMPUTX('PATH',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DAILY\QFUND5\Title",'G');
    CALL SYMPUTX('PATHTWO',"E:\SHARED\CADA\SAS DATA\DATAMART\SKYNET REDESIGN BULKLOAD LOGS\DAILY\QFUND5\Title\Dir2",'G');
%RUNQUIT(&job,&sub11);

PROC SQL;
    INSERT INTO SKY.DAILYSUM_DATAMART_QF5T (BULKLOAD=YES BL_LOG="&PATH.\BL_&DATE..LOG" BL_DELETE_DATAFILE=YES 
                                                   BL_DEFAULT_DIR="&PATHTWO.")
    SELECT 
        *
    FROM DAILY_SUMMARY_ALL_QF5T_1
...
SYSTASK COMMAND "'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SAS.EXE'
				 '&DAILY_FILE_PATH.\TRANSPOSE QFUND5_TITLE.SAS'
				 -LOG '&DAILY_LOGPATH.\TRANSPOSE_QFUND5_TITLE_&TIMESTAMP..LOG'
				 -CONFIG 'C:\PROGRAM FILES\SASHOME\SASFOUNDATION\9.4\SASV9.CFG'"
TASKNAME=TRANSPOSE_QF5_TITLE
STATUS=TRANSPOSE_QF5_TITLE;

/*UPLOAD QF5TLP*/
%INCLUDE "&DAILY_FILE_PATH.\DAILYSUM_UPLOAD_QFUND5_T.SAS";
...
PROC SQL;
	...
	EXECUTE(UPDATE BIOR.O_DAILY_SUMMARY_ALL
			...
			WHERE INSTANCE = 'QFUND5-6' AND PRODUCT = 'TITLE'
			)
	BY ORACLE;
	DISCONNECT FROM ORACLE;
QUIT;

WAITFOR _ALL_ TRANSPOSE_QF5_TITLE;
